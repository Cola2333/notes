...
* Expiry
  ...
  ![图片1](C:\WJJ\StudyInUSC\spring踩坑与学习\Spring Boot-高级篇-images\图片1.png)
...
* Spring提供了各种xxxCache的实现；如RedisCache，EhCacheCache ,
  ...
  ![图片2](C:\WJJ\StudyInUSC\spring踩坑与学习\Spring Boot-高级篇-images\图片2.png)
...
​	![图片3](C:\WJJ\StudyInUSC\spring踩坑与学习\Spring Boot-高级篇-images\图片3.png)

2. 应用解耦

   在下单时库存系统不能正常使用。也不影响正常下单，因为下单后，订单系统写入消息队列就不再关心其他的后续操作了。实现订单系统与库存系统的应用解耦

   ![图片4](C:\WJJ\StudyInUSC\spring踩坑与学习\Spring Boot-高级篇-images\图片4.png)
...
3. 流量削峰
   ...
   ![图片5](C:\WJJ\StudyInUSC\spring踩坑与学习\Spring Boot-高级篇-images\图片5.png)
...
* **Exchange**
  ...
  - 交换器，将生产者消息路由给服务器中的队列 和消息队列是多对多关系
...
* Broker
  ...
  - 消息队列服务器实体
    ...
    ![image-20200520163849098](C:\WJJ\StudyInUSC\spring踩坑与学习\Spring Boot-高级篇-images\image-20200520163849098.png)

生产者将消息发给消息代理（服务器）里的虚拟主机，虚拟机将消息给指定的交换机，交换机根据消息的路由件将消息路由给消息队列，路由规则由binding决定（暂时理解为当路由件与binding一致时 传给消息队列）。消息到达消息队列之后，消费者就可以从消息队列中取出消息。消费者与消息队列建立理解，为了提升效率，开启多路复用，开启很多管道，消息队列里的消息通过管道返回给消费者
...
3. topic
   ...
   ![Snipaste_2020-04-09_14-11-13](C:\WJJ\StudyInUSC\spring踩坑与学习\Spring Boot-高级篇-images\Snipaste_2020-04-09_14-11-13.png)
...
![Snipaste_2020-04-11_23-25-26](C:\WJJ\StudyInUSC\spring踩坑与学习\Spring Boot-高级篇-images\Snipaste_2020-04-11_23-25-26.png)
...
同样的，可以使用 `DELETE` 命令来删除文档

使用 `HEAD` 指令来检查文档是否存在。不会有响应数据，但是如果存在响应状态为200
