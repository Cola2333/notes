...
| 名称                  | 解释                                   | 命令                    |
| --------------------- | -------------------------------------- | ----------------------- |
| DDL（数据定义语言）   | 定义和管理数据对象，如数据库数据表     | CREATE, DROP, ALTER     |
| DML（数据操作语言）   | 用于操作数据对象中包含的数据           | INSERT, UPDATE, DELETE  |
| DQL（数据查询语言）   | 用于查询数据库中的数据                 | SELECT                  |
| DCL（数据库控制语言） | 用于管理数据库，包括管理权限及数据更改 | GRANT, commit, rollback |
...
记得设置数据表的类型```ENGINE=InnoDB DEFAULT CHARSET=utf8```
...
如果两张表具有公共关键字，其中该关键字在一个关系中是主关键字，那么这个公共关键字被称为另一个关系的外键。

以另一个表的外键作主关键字的表被称为**主表**，具有此外键的表被称为主表的**从表**。**在一对多关系中，多的那一边储存外键。**

在实际操作中，将一个表的值放入第二个表来表示关联，所使用的值是第一个表的主键值(在必要时可包括复合主键值)。此时，第二个表

中保存这些值的属性称为外键(**foreign key**)。
...
[参考](https://mp.weixin.qq.com/s?__biz=Mzg2NTAzMTExNg==&mid=2247484207&idx=1&sn=2ba333652e06e6b2dcfa949de09aea70&scene=19#wechat_redirect)
...
[参考](https://mp.weixin.qq.com/s?__biz=Mzg2NTAzMTExNg==&mid=2247484217&idx=1&sn=b2a3463dc50417a571bd551749701065&scene=19#wechat_redirect)
...
隔离状态执行事务，使它们好像是系统在给定时间内执行的唯一操作。如果有两个事务，运行在相同的时间内，执行相同的功能，事务的隔离性将**确保每一事务在系统中认为只有该事务在使用系统**。这种属性有时称为串行化，为了防止事务操作间的混淆，必须串行化或序列化请求，使得在同一时间仅有一个请求用于同一数据。

**持久性(Durable)**

在事务完成提交以后，该事务对数据库所作的更改便持久的保存在数据库之中，不可再回滚。
...
1. 提高查询速度
2. 使用分组和排序子句进行数据检索时 , 可以显著减少分组和排序的时间
3. 全文检索字段进行搜索优化.
4. 确保数据的唯一性
5. 可以加速表和表之间的连接 , 实现表与表之间的参照完整性

### 分类

- 主键索引 (Primary Key)

  + 主键：某一个属性（组）能唯一标识一条记录
...
- 常规索引 (Index)
  ...
  CREATE TABLE `result`(
     -- 省略一些代码
    KEY `ind` (`studentNo`,`subjectNo`) -- 创建表时添加
...
-- 对于表中的每个记录行，MATCH() 返回一个相关性值。即，在搜索字符串与记录行在 MATCH() 列表中指定的列的文本之间的相似性尺度。
...
EXPLAIN SELECT * FROM app_user WHERE name = '用户9999'\G -- \G标识把结果改成纵向结构
```
