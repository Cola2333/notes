...
m为大于1的一个正整数，称为B-Tree的阶数。

h为一个正整数，称为B-Tree的高度。

每个非叶子节点由n - 1个key和n个指针组成，其中m/2 <= n <= m（也就是说关键字个数[m/2 - 1, m - 1]，孩子个数[m/2, m]）。
...
如果某个指针在节点node的左右相邻key分别是keyi和keyi+且不为null，则其指向节点的所有key小于v(keyi+1))且大于v(keyi)。
...
**由于B-Tree的特性，在B-Tree中按key检索数据的算法非常直观：首先从根节点进行二分查找，如果找到则返回对应节点的data，否则对指针指向的响应区间的节点递归进行查找，直到找到节点或找到null指针，前者查找成功，后者查找失败。B-Tree上查找算法的伪代码如下：**

```java
//伪代码
BTree_Search(node, key) {
    if(node == null) return null;
    foreach(node.key) // 遍历节点中每一个key值 从小到大查找（若比当前节点值大 则循环继续 若小 则会则会进行递归查找下一层）
    {
        // 找到所查key值 返回
        if(node.key[i] == key) return node.data[i]; 
        	//所查key值小于当前key值 说明要找的key值不在当前层了 递归查找指针指向的下一层的节点
            if(node.key[i] > key) return BTree_Search(point[i]->node); 
    }
    // 所查找的key值大于本层所有key值 递归查找下一层最右侧节点
...
关于B-Tree有一系列有趣的性质，例如一个阶数为m的B-Tree，设其索引N个key，则其树高h的上限为$log_mN+1$，检索一个key，其查找节点个数的渐进复杂度为$O(log_mN)$。从这点可以看出，B-Tree是一个非常有效率的索引数据结构。
...
1、每个非叶子节点由n 个key和n个指针组成，其中m/2 <= n <= m - 1。
...
由于并不是所有节点都具有相同的域（非叶子节点没有data域），因此**B+Tree中叶节点和内节点一般大小不同**。这点与B-Tree不同，虽然B-Tree中不同节点存放的key和指针可能数量不一致，但是每个节点的域和上限是一致的，所以在实现中**B-Tree往往对每个节点申请同等大小的空间**。
...
一个磁盘由大小相同且同轴的圆形盘片组成，每个盘片包含两个面，磁盘可以转动（各个磁盘必须同步转动）。每个盘面都对应地有一个读/写磁头。磁头不能转动，但是可以沿磁盘半径方向运动（实际是斜切向运动），每个磁头同一时刻也必须是同轴的，即从正上方向下看，所有磁头任何时候都是重叠的（不过目前已经有多磁头独立技术，可不受此限制）。
...
盘面中一圈圈灰色同心圆为一条条磁道，所有半径相同的磁道组成一个柱面。从圆心向外画直线，可以将磁道划分为若干个弧段，每个磁道上一个弧段被称之为一个扇区（图践绿色部分）。**扇区是磁盘的最小组成单元**。为了简单起见，我们下面假设磁盘只有一个盘片和一个磁头。
...
B-Tree中一次检索最多需要h-1次I/O（根节点常驻内存），渐进复杂度为$O(h) = O(log_mN)$。一般实际应用中，m是非常大的数，因此h非常小（通常不超过3）。

综上所述，用B-Tree作为索引结构效率是非常高的。

而红黑树这种结构，h明显要深的多。由于逻辑上很近的节点（父子）物理上可能很远，无法利用局部性，所以红黑树的I/O渐进复杂度也为O(h)，效率明显比B-Tree差很多。

上文还说过，B+Tree更适合外存索引，原因和内节点出度d有关（可以简单理解成阶数除2等于度）。从上面分析可以看到，d越大索引的性能越好，而出度的上限取决于节点内key和data的大小：

$dmax=floor(pagesize/(keysize+datasize+pointsize))$

$floor$表示向下取整。由于B+Tree内节点去掉了data域，因此可以拥有更大的出度，拥有更好的性能。
...
另一种不建议建索引的情况是索引的选择性较低。所谓索引的选择性（Selectivity），是指不重复的索引值（也叫基数，Cardinality）（我简单理解成同一列下不重复的值得个数）与表记录数（#T）的比值：
...
e的选择性不足0.0001（精确值为0.00001579），所以实在没有什么必要为其单独建索引。
...
前缀索引兼顾索引大小和查询速度，但是其缺点是不能用于ORDER BY和GROUP BY操作，也不能用于Covering index（即当索引本身包含查询所需全部数据时，不再访问数据文件本身）（也就是说直接把索引当做结果？）。
